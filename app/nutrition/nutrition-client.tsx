...
export default function NutritionClient({ user }: { user: ProfileUser }) {
  ...
  const eatenCalories = meals.reduce((sum, meal, index) => sum + (completed[index] ? meal.calories : 0), 0);

  useEffect(() => {
    const timer = window.setTimeout(() => setToday(new Date().toLocaleDateString("en-CA")), 0);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        if (Array.isArray(stored) && stored.length === meals.length) setCompleted(stored.map(Boolean));
