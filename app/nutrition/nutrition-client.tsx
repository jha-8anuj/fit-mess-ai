...
export default function NutritionClient({ user }: { user: ProfileUser }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => { const date = new Date(); setToday(date.toLocaleDateString("en-CA")); setDayNumber(date.getDate() % mealSets.length); }, 0);
